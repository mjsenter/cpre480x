...
GLfloat GLT_WHITE[3] = {1.0f, 1.0f, 1.0f};
GLfloat GLT_BLACK[3] = {0.0f, 0.0f, 0.0f};
GLfloat GLT_YELLOW[3] = {1.0f, 1.0f, 0.0f};
...
GLboolean bDisableExt = GL_FALSE;   /* disable extensions */
GLfloat *fpsColor = GLT_WHITE;      /* color of the FPS counter */
unsigned *indexQueue = NULL;
color_t *colorQueue = NULL;
vertex_t *vertexQueue = NULL;
unsigned indexQueueIndex = 0;
unsigned vertexQueueIndex = 0;
unsigned colorQueueIndex = 0;
...
int initSGP(void) __attribute__((constructor));
int initSGP(void)
{



  int returnValue =  configSGPTransmit( &config, 1 );
...
GLboolean gltraceInit()
{
	...
	char		loctime[32];
        ...
        int error = initSGP();
        if(error < 0) {
           printf("Error setting up SGP interface\n");
        }


        indexQueue = (unsigned int *) malloc(sizeof(unsigned int) * 32);
        colorQueue = (color_t *) malloc(sizeof(color_t) * 32);
        // set default color to white (all 1 in fix point frac S1.30)
        colorQueue[0]= 0x00ffffff;
        vertexQueue = (vertex_t *) malloc(sizeof(vertex_t) * 32);
        indexQueueIndex = 0;
        vertexQueueIndex = 0;
        colorQueueIndex = 0;
	...
	strcpy(INI_pathname,"GLTRACE.INI");
	...
	if (INI != NULL)
	{
		...
		while (fgets(line, sizeof(line)-1, INI) != NULL)
		{
			...
			if (strcmp(line, "[Configuration]") == 0)  { s = CFG; continue; }
			if (strcmp(line, "[Keys]") == 0)            { s = KEY; continue; }
			if (strcmp(line, "[Implementation]") == 0) { s = IMP; continue; }
			if (strcmp(line, "[Exclude]") == 0)        { s = EXC; continue; }
			if (strcmp(line, "[Optional]") == 0)       { s = OPT; continue; }
			if (strcmp(line, "[Output]") == 0)         { s = OUTP; continue; }
			
			switch (s) {

			case CFG:
				if (strcmp("DisableExt", line) == 0)
                {
                    bDisableExt = GL_TRUE;
					break;
				}
            ...
            case IMP:
				...
				strncpy(OGL_pathname, line, PATH_MAX-1);
                OGL_pathname[PATH_MAX-1] = (char)0;
				break;
				
            case OUTP:
				
				text_file = fopen(line, "w");
				break;
	...
	{
		print("Couldn't find OpenGL driver glTrace\n");
		return GL_FALSE;
	}
	...
	print("\n");
	print("-------------------------------------------------------------------------------\n");
	print("GLTrace version %s, copyright 1999-2002 by Phil Frisbie, Jr. phil@hawksoft.com\n", VERSION);
	print("GLTrace comes with ABSOLUTELY NO WARRANTY. This is free software, and you are\n");
	print("welcome to redistribute it under certain conditions; see the file license.txt.\n");
	print("-------------------------------------------------------------------------------\n");
