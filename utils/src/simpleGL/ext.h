...
typedef struct
{
    ...
    BOOL (GLAPIENTRY *wglRestoreBufferRegionARB) (HANDLE hRegion, int x, int y, int width, int height, int xSrc, int ySrc);
#endif
    /* ARB_multisample */
...
int show_status_EXT[ARY_CNT(GLN_EXT)];
...
#define START_EXT(X)	\
    if(bCountCalls == GL_TRUE) nCallCountEXT[FP_##X]++;\
	if(bLogCalls == GL_TRUE)\
	{\
		if(((show_status_EXT[FP_##X] == SHOW_ALWAYS) && (bOnOffKeydown == GL_TRUE)) \
...
int GetEXTProcIndex(LPCSTR name)
{
	int i;
...
char *getWGLString(int attribute)
{
    int i;
...
BOOL GLAPIENTRY wglGetPixelFormatAttribivARB(HDC hdc, int iPixelFormat, int iLayerPlane, UINT nAttributes, const int *piAttributes, int *piValues)
{
	...
	result = GLV_EXT.wglGetPixelFormatAttribivARB(hdc, iPixelFormat, iLayerPlane, nAttributes, piAttributes, piValues);
...
BOOL GLAPIENTRY wglGetPixelFormatAttribfvARB(HDC hdc, int iPixelFormat, int iLayerPlane, UINT nAttributes, const int *piAttributes, FLOAT *pfValues)
{
	...
	result = GLV_EXT.wglGetPixelFormatAttribfvARB(hdc, iPixelFormat, iLayerPlane, nAttributes, piAttributes, pfValues);
...
BOOL GLAPIENTRY wglChoosePixelFormatARB(HDC hdc, const int *piAttribIList, const FLOAT *pfAttribFList, UINT nMaxFormats, int *piFormats, UINT *nNumFormats)
{
	...
	result = GLV_EXT.wglChoosePixelFormatARB(hdc, piAttribIList, pfAttribFList, nMaxFormats, piFormats, nNumFormats);
