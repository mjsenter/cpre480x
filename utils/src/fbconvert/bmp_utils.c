...
/*****************************************************************************
 * bmp_utils.c - utility functions for reading and writing to bmp files.
 *
 *
 * NOTES:
...
unsigned int **bmp_to_array(config_type *config, bmp_file_info *bmp) {
  ...
  fread(bmp->h1, sizeof(bmp_magic), 1, config->infile);
  fread(bmp->h2, sizeof(bmp_header), 1, config->infile);
  fread(bmp->h3, sizeof(dib_header), 1, config->infile);
  ...
  if ((bmp->h3->bitspp != config->depth) || 
    ...
    fprintf(stderr, "Warning: %s configuration and header information in %s ", 
	    EXEC_NAME, config->infile_name);
  ...
  if (bmp->h3->bitspp < 16) {
    ...
    fread(bmp->h4, sizeof(unsigned int), num_colors, config->infile);
  }
  ...
  for (i = 0; i < theight; i++) {
    fread(tpixels, 1, rowbytes, config->infile);
	...
	break;
      case 16:
	for (j = 0; j < twidth; j++) {
	  pixels[theight-i-1][j] = (tpixels[2*j] << 0) | 
	                           (tpixels[2*j+1] << 8);
	}
	break;
      case 24:
	for (j = 0; j < twidth; j++) {
	  pixels[theight-i-1][j] = (tpixels[3*j] << 0) | 
	                           (tpixels[3*j+1] << 8) | 
	                           (tpixels[3*j+2] << 16);
	}
    ...
    if (rowbytes % 4 != 0) {
      fread(extra_pixels, 1, 4-(rowbytes%4), config->infile);
    }
...
void array_to_bmp(config_type *config, unsigned int **pixels, bmp_file_info *bmp) {
  ...
  for (i = 0; i < theight; i++) {
	...
	for (j = 0; j < rowbytes; j+=3) {
	  ...
	  tpixels[j+1] = (pixels[theight-i-1][j/3] >> 8) & 0xFF;
	...
	for (j = 0; j < rowbytes; j+=4) {
	  tpixels[j] = (pixels[theight-i-1][j/3] >> 0) & 0xFF;
	  tpixels[j+1] = (pixels[theight-i-1][j/3] >> 8) & 0xFF;
	  tpixels[j+1] = (pixels[theight-i-1][j/3] >> 16) & 0xFF;
	  tpixels[j+3] = (pixels[theight-i-1][j/3] >> 24) & 0xFF;
	}
	break;
      default:
	break;
